...
zb_nwk_hdr_t nwk_hdr_size;

zb_uint8_t ZB_NWK_CMD_FRAME_GET_CMD_ID(zb_buf_t * buf, zb_uint8_t nwk_hdr_size)
{
	return (*(zb_nwk_cmd_t *)(ZB_BUF_BEGIN(buf) + nwk_hdr_size));
}

zb_uint8_t * ZB_NWK_CMD_FRAME_GET_CMD_PAYLOAD(zb_buf_t * buf, zb_uint8_t nwk_hdr_size)
{
	return( (zb_uint8_t *)(ZB_BUF_BEGIN(buf) + nwk_hdr_size) + 1);
}

zb_ushort_t ZB_NWK_FULL_HDR_SIZE(zb_bool_t is_multicast)
{
	zb_ushort_t nwk_full_hdr = 0;

	/*TODO : this line is not supported by SDCC compiler*/
//	nwk_full_hdr = ZB_OFFSETOF(zb_nwk_hdr_t, mcast_control);
	nwk_full_hdr = sizeof(nwk_hdr_size.mcast_control);
...
{
	zb_ushort_t nwk_half_hdr = 0;

	nwk_half_hdr = sizeof(nwk_hdr_size.src_ieee_addr);
...
{
	zb_ushort_t nwk_short_hdr = 0;

	nwk_short_hdr = sizeof(nwk_hdr_size.dst_ieee_addr);

	if(is_multicast){
		nwk_short_hdr += 1;
	}

	return nwk_short_hdr;
}
...
#ifdef  ZB_NWK_MESH_ROUTING
  ZG->nwk.nib.aps_rreq_addr = 0xffff;//-1;
#endif
  ...
  if (ent)
  {
    ...
    */
                 ...
                 && ent->src_addr != ZB_PIB_SHORT_ADDRESS())) )
    {
      ...
      /* Optimization could be done here, we could use ent->buf for the
...
void zb_nwk_do_leave(zb_uint8_t param, zb_uint8_t rejoin)
{
  ...
  if (rejoin)
  {
    ...
    req->scan_duration = ZB_DEFAULT_SCAN_DURATION; /* configure it somehow? */
...
//TODO : this functions seems to be trouble with compile error
/*
...
void zb_nlde_data_request(zb_uint8_t param)
{
  ...
  /* if not broadcast or multicast, send source and destination ieee
   * address.
...
void zb_mcps_data_indication(zb_uint8_t param)
{
  ...
  /* if frame is from non-joined device, drop it (how to check it??) */
  ...
  */
...
#endif
  {
    ...
    else
    {
      ...
      if ( buf->u.hdr.status != MAC_SUCCESS )
      {
        /* fix me: we could rewrite status here and send wrong indication */
        /* send status.indication */
        ZG->nwk.handle.status_ind = buf->u.hdr.status;
        ZG->nwk.handle.status_addr = dest_addr;
